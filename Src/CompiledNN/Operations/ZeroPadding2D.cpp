...
namespace NeuralNetwork
{
  namespace CompiledNNImpl
  {
    void ZeroPadding2DCompiler::compile(X86Assembler& a, ActivationFunctionHandler& afHandler, const TensorPointerXf& input, const TensorPointerXf& output) const
    {
      ...
      a.mov(a.zdi(), imm_ptr<const float*>(output.data() + ((output.dims(1) * p.padding[ZeroPadding2DLayer::TOP] + p.padding[ZeroPadding2DLayer::LEFT]) * output.dims(2))));
      if(input.dims(1) * input.dims(2) % 4 == 0)
      {
        a.mov(a.zax(), imm_u(input.dims(0)));
        ...
        a.add(a.zdi(), imm_u((p.padding[ZeroPadding2DLayer::LEFT] + p.padding[ZeroPadding2DLayer::RIGHT]) * output.dims(2) * sizeof(float)));
      ...
      a.mov(a.zsi(), imm_ptr<const float*>(output.data()));
      ...
      a.add(a.zsi(), imm_u((input.dims(0) * (input.dims(1) + p.padding[ZeroPadding2DLayer::LEFT] + p.padding[ZeroPadding2DLayer::RIGHT]) * input.dims(2) + remainingElements) * sizeof(float)));
      ...
      a.mov(a.zsi(), imm_ptr<const float*>(output.data() + p.padding[ZeroPadding2DLayer::TOP] * (input.dims(1) + p.padding[ZeroPadding2DLayer::LEFT] + p.padding[ZeroPadding2DLayer::RIGHT]) * input.dims(2)));
      a.mov(a.zcx(), imm_u(input.dims(0)));
      ...
      a.add(a.zsi(), imm_u((input.dims(1) + p.padding[ZeroPadding2DLayer::LEFT] + p.padding[ZeroPadding2DLayer::RIGHT]) * input.dims(2) * sizeof(float)));
