...
namespace NeuralNetwork
{
  ...
  {
    void UpSampling2DCompiler::compile(X86Assembler& a, ActivationFunctionHandler& afHandler, const TensorPointerXf& input, const TensorPointerXf& output) const
    {
      ASSERT(input.rank() == 3);
      ...
      if(input.dims(1) > 1)
      {
        ...
        a.mov(a.zcx(), imm_u(input.dims(1)));
        a.bind(columnLoop);
      }
      ...
      a.add(a.zsi(), imm_u(input.dims(2) * sizeof(float)));
      ...
      a.add(a.zdi(), imm_u(p.size[1] * input.dims(2) * sizeof(float)));
      ...
      {
        if(p.size[0] > 1)
          a.add(a.zdi(), imm_u((p.size[0] - 1) * rowStep));
